/* VerOS Flow — melhorias finais aprovadas
   ...
   2) Kanban com rolagem horizontal.
...
(function(){
  ...
  function role(){
    return norm(window.STATE?.currentUser?.perfil || window.STATE?.currentProfile || '');
  }
  function currentSol(){
  ...
  function ensureStyles(){
    ...
    st.textContent=`
      /* Kanban: mantém todas as colunas em uma única linha e permite rolagem horizontal */
      .kanban-board,.kanban-container,.kanban-grid,[class*="kanban-board"],[class*="kanban-container"],[class*="kanban-grid"]{
      ...
      .kanban-board,.kanban-container,.kanban-grid{min-width:0;}
      .kanban-board > *, .kanban-container > *, .kanban-grid > *{min-width:310px;}
  ...
  function patchKanban(){
    document.querySelectorAll('[class*="kanban"],.kanban-board,.kanban-container,.kanban-grid').forEach(el=>{
      el.style.overflowX='auto';
      el.style.overflowY='hidden';
      el.style.maxWidth='100%';
      Array.from(el.children).forEach(c=>{ c.style.minWidth='310px'; });
    });
  }
  ...
  function renderEstimate(){
    const old=document.getElementById('vfDeliveryEstimate');
    if(old) old.remove();
    const s=currentSol();
    if(!s) return;
    ensureStyles();
    ...
    box.id='vfDeliveryEstimate';
    box.className='vf-delivery-estimate';
    ...
    if(e.complete){
      ...
      const faltam=[];
      if(e.producao===null) faltam.push('tempo de produção');
      if(e.entrega===null) faltam.push('prazo de entrega');
  ...
  async function concluirProducao(){
    const s=currentSol();
    if(!s || !canAdmForSol(s)) return;
    const status=norm(s.status);
    if(!status.includes('PRODUCAO')) return;
    ...
    if(error){
      console.error('[VerOS Flow] erro ao concluir produção:',error);
      if(window.App?.toast) App.toast('Erro ao concluir produção',error.message||'Tente novamente.','err');
      return;
    }
    s.status=nextStatus;
  ...
  function renderAdmProductionAction(){
    const old=document.getElementById('vfAdmProductionAction');
    if(old) old.remove();
    const s=currentSol();
    if(!s || !canAdmForSol(s)) return;
    const st=norm(s.status);
    if(!st.includes('PRODUCAO') || st.includes('FATURAMENTO')) return;
    ensureStyles();
    const box=document.createElement('section');
    box.id='vfAdmProductionAction';
    ...
    const est=document.getElementById('vfDeliveryEstimate');
    const main=document.querySelector('#appShell .main-area');
    if(est && est.parentNode) est.parentNode.insertBefore(box,est.nextSibling);
  ...
  window.VEROS_FINAL_ENHANCEMENTS={version:'1.0',estimate};
})();
