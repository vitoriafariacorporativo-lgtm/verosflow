...
(function(){
  ...
  function renderFriendlyBreadcrumb(){
    ...
    const friendly=String(s.numero_pedido_mobi ?? s.numeroPedidoMobi ?? s.numero_pedido ?? s.numeroPedido ?? s.codigo ?? s.nome ?? '').trim();
    if(!friendly)return;
    ...
    for(const textNode of nodes){
      if(String(textNode.nodeValue||'').toLowerCase().includes(internalId.toLowerCase())){
        ...
        breadcrumb.setAttribute('title',`Pedido: ${friendly}`);break;
      }
    }
  }
  ...
  function isRemarksApproval(select){
    const value=norm(select.value);
    const text=norm(select.options?.[select.selectedIndex]?.textContent||'');
    return value.includes('APROVAR_COM_RESSALVAS') || text.includes('APROVAR_COM_RESSALVAS');
  }

  function findApprovalContainer(select){
    return select.closest('.field,.form-group,.form-field,.approval-field,.card,form') || select.parentElement;
  }

  function renderCreditRemarks(){
    document.querySelectorAll('select').forEach(select=>{
      if(!isRemarksApproval(select)){
        const existing=document.getElementById(`vfCreditRemarks_${select.dataset.vfRemarksId||''}`);
        if(existing)existing.remove();
        return;
      }

      if(!select.dataset.vfRemarksId)select.dataset.vfRemarksId=`${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
      const id=`vfCreditRemarks_${select.dataset.vfRemarksId}`;
      let box=document.getElementById(id);
      if(box)return;
      ensureStyles();
      box=document.createElement('div');box.id=id;box.className='vf-credit-remarks';
      const textareaId=`vfCreditRemarksText_${select.dataset.vfRemarksId}`;
      ...
      const container=findApprovalContainer(select);
      if(container?.parentNode)container.parentNode.insertBefore(box,container.nextSibling);else select.insertAdjacentElement('afterend',box);
    });
  }

  function validateCreditRemarks(target){
    const select=target?.matches?.('select')?target:null;
    const remarksBox=target?.closest?.('.vf-credit-remarks');
    if(remarksBox){const ta=remarksBox.querySelector('textarea');return!!ta?.value.trim();}
    if(select&&!isRemarksApproval(select))return true;
    const boxes=document.querySelectorAll('.vf-credit-remarks');let valid=true;
    boxes.forEach(box=>{const ta=box.querySelector('textarea');if(ta&&!ta.value.trim()){valid=false;ta.classList.add('vf-invalid');box.querySelector('.vf-credit-remarks-required')?.classList.add('show');}});
    return valid;
  }
  ...
  function bindCreditRemarks(){
    ...
    document.addEventListener('change',event=>{if(event.target.matches('select')){renderCreditRemarks();}});
    document.addEventListener('input',event=>{if(event.target.matches('.vf-credit-remarks textarea')){event.target.classList.remove('vf-invalid');event.target.parentElement.querySelector('.vf-credit-remarks-required')?.classList.remove('show');}});
    document.addEventListener('click',event=>{
      const btn=event.target.closest('button');if(!btn)return;
      const label=norm(btn.textContent||'');
      if(label.includes('APROVAR_COM_RESSALVAS')){
        if(!validateCreditRemarks()){event.preventDefault();event.stopPropagation();}
      }
    },true);
  ...
  window.addEventListener('load',refresh);window.addEventListener('verosflow:render',refresh);
  window.VEROS_FINAL_ENHANCEMENTS={version:'1.5',estimate};
})();
